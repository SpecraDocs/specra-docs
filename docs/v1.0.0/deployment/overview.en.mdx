...
<Steps>
  <Step title="Install the CLI">
    Install the Specra CLI globally:
...
```
  ...
  <Step title="Authenticate">
    Log in with your API token. Generate one from your [Specra dashboard](https://specra-docs.com/dashboard) under **Settings > API Tokens**.

```bash
specra login
```

    When prompted, paste your API token. The token is saved to `~/.specra/config.json`.
...
|----------|---------------|------------------|
| Specra Cloud | `specra deploy` (handles build) | Managed |
...
<CardGrid cols={3}>
  <Card
    icon="cloud"
    title="Specra Cloud"
    description="One-command deploy with the CLI"
