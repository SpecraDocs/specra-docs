...
<Steps>
  <Step title="CLI installieren">
    Installieren Sie das Specra CLI global:
...
```
  ...
  <Step title="Authentifizieren">
    Melden Sie sich mit Ihrem API-Token an. Generieren Sie einen in Ihrem [Specra-Dashboard](https://specra-docs.com/dashboard) unter **Einstellungen > API-Tokens**.

```bash
specra login
```

    Fügen Sie Ihren API-Token ein, wenn Sie dazu aufgefordert werden. Der Token wird in `~/.specra/config.json` gespeichert.
  </Step>
...
```
    ...
    Das CLI baut Ihr Projekt und stellt es auf Specra Cloud bereit. Sie erhalten eine URL wie `ihr-projekt.docs.specra-docs.com`.
  </Step>
</Steps>
...
|-----------|--------------|--------------------|
| Specra Cloud | `specra deploy` (verwaltet den Build) | Verwaltet |
