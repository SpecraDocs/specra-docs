...
<Steps>
  <Step title="Installer le CLI">
    Installez le CLI Specra globalement :
...
```
  ...
  <Step title="S'authentifier">
    Connectez-vous avec votre jeton API. Générez-en un depuis votre [tableau de bord Specra](https://specra-docs.com/dashboard) sous **Paramètres > Jetons API**.

```bash
specra login
```

    Collez votre jeton API lorsque demandé. Le jeton est sauvegardé dans `~/.specra/config.json`.
  </Step>
...
```
    ...
    Le CLI construit votre projet et le déploie sur Specra Cloud. Vous obtiendrez une URL comme `votre-projet.docs.specra-docs.com`.
  </Step>
</Steps>
...
|------------|-------------------|----------------------|
| Specra Cloud | `specra deploy` (gère le build) | Géré |
