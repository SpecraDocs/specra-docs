...
<Steps>
  <Step title="Instalar el CLI">
    Instale el CLI de Specra globalmente:
...
```
  ...
  <Step title="Autenticarse">
    Inicie sesión con su token API. Genere uno desde su [panel de Specra](https://specra-docs.com/dashboard) en **Configuración > Tokens API**.

```bash
specra login
```

    Pegue su token API cuando se le solicite. El token se guarda en `~/.specra/config.json`.
  </Step>
...
```
    ...
    El CLI construye su proyecto y lo despliega en Specra Cloud. Obtendrá una URL como `su-proyecto.docs.specra-docs.com`.
  </Step>
</Steps>
...
|------------|------------------------|----------------------|
| Specra Cloud | `specra deploy` (gestiona el build) | Gestionado |
